...
type FlightResponseBody = {
	departing: FlightOffers[];
	returning: FlightOffers[];
};
...
export default async function handler(
	...
	try {
		...
		try {
			...
			// Get pricing for the departing and return flights with the limit
			const departingOffers = departingResponse.data.slice(0, limit / 2);
			const returnOffers = returnResponse.data.slice(0, limit / 2);
			...
			const transformedResult = transformAmadeusData(result);
...
function transformAmadeusData(data: any): {
	departing: FlightOffers[];
	returning: FlightOffers[];
} {
	const airlinesMap = new Map(
		airlinesData.map((airline: any) => [
			airline.id,
			{ name: airline.name, logo: airline.logo },
		])
	);
	let departing: FlightOffers[] = [];
	let returning: FlightOffers[] = [];

	["departing", "return"].forEach((key) => {
		data[key].forEach((flightOffer: any) => {
			let startingTime = "";
			let endingTime = "";
			let totalDurationMinutes = 0;
			let price = parseInt(flightOffer.price.total);
			let airlineCode = "";
			let airlineName = "";
			let airlineImage = "";
	...
	return { departing: departing, returning: returning };
}
